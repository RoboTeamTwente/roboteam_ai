...
void Visualizer::drawFieldLines(const rtt::world::Field &field, QPainter &painter) {
    ...
    for (const auto &fieldLine : field.getFieldLines()) {
        ...
        painter.drawEllipse(QPointF(screenPos.x, screenPos.y), centercircle.radius * factor,
...
// draw a single robot
    ...
    Visualizer::drawRobot(QPainter &painter, rtt::world::view::RobotView robot, bool ourTeam, std::string role) {
        ...
        if (showAngles) {
            Vector2 angle = toScreenPosition(
                    {robot->getPos().x + cos(robot->getAngle()) / 3, robot->getPos().y + sin(robot->getAngle()) / 3});
            QPen pen;
            pen.setWidth(2);
        ...
        if (showVelocities) {
            Vector2 vel = toScreenPosition(
                    {robot->getPos().x + robot->getVel().x, robot->getPos().y + robot->getVel().y});
...
// Handle mousePressEvents
    ...
    void Visualizer::drawTacticColorForRobot(QPainter &painter, rtt::world::view::RobotView robot) {
        ...
        painter.setPen(Qt::transparent);
        painter.setBrush(c);
        painter.drawEllipse(qrobotPosition, Constants::TACTIC_COLOR_DRAWING_SIZE(),
    ...
    void Visualizer::drawArrows(QPainter &painter, std::vector<Vector2> points, double factor, double maxSize,
                                bool closedArrow) {
        if (points.size() >= 2) {
            for (int i = 1; i < points.size(); i += 2) {
                ...
                if (closedArrow) {
                    painter.drawLine(arrowStartOnScreen.x, arrowStartOnScreen.y, startPointOnScreen.x,
                                     startPointOnScreen.y);
                    painter.drawLine(arrowEndOnScreen.x, arrowEndOnScreen.y, pointyBitRightOnScreen.x,
                                     pointyBitRightOnScreen.y);
                    painter.drawLine(arrowEndOnScreen.x, arrowEndOnScreen.y, pointyBitLeftOnScreen.x,
                                     pointyBitLeftOnScreen.y);
                    painter.drawLine(pointyBitRightOnScreen.x, pointyBitRightOnScreen.y, pointyBitLeftOnScreen.x,
                                     pointyBitLeftOnScreen.y);
                } else {
                    painter.drawLine(arrowStartOnScreen.x, arrowStartOnScreen.y, arrowEndOnScreen.x,
                                     arrowEndOnScreen.y);
                    painter.drawLine(arrowEndOnScreen.x, arrowEndOnScreen.y, pointyBitRightOnScreen.x,
                                     pointyBitRightOnScreen.y);
                    painter.drawLine(arrowEndOnScreen.x, arrowEndOnScreen.y, pointyBitLeftOnScreen.x,
    ...
    void
    Visualizer::drawRealLifeSizedPoints(QPainter &painter, std::vector<Vector2> points, double width, double height) {
