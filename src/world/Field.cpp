...
bool Field::pointIsInField(const Vector2 &point, double margin) {
    ...
    return (point.x <= field.get(RIGHTMOST_X) - margin && point.x >= field.get(LEFTMOST_X) + margin && point.y <= field.get(TOPMOST_Y) - margin &&
            point.y >= field.get(BOTTOMMOST_Y) + margin);
}
...
std::vector<std::pair<Vector2, Vector2>> Field::mergeBlockades(std::vector<std::pair<Vector2, Vector2>> blockades) {
    ...
    std::sort(blockades.begin(), blockades.end(), [](const std::pair<Vector2, Vector2> &a, const std::pair<Vector2, Vector2> &b) { return a.first.y < b.first.y; });
...
std::pair<Vector2, Vector2> Field::getGoalSides(bool ourGoal) {
    ...
    if (ourGoal) {
        return std::make_pair(_field.get(OUR_BOTTOM_GOAL_SIDE), _field.get(OUR_TOP_GOAL_SIDE));
    } else {
        return std::make_pair(_field.get(THEIR_BOTTOM_GOAL_SIDE), _field.get(THEIR_TOP_GOAL_SIDE));
    }
}
...
Vector2 Field::getPenaltyPoint(bool ourGoal) {
    if (ourGoal) {
        return get_field().get(LEFT_PENALTY_POINT);
    } else {
        return get_field().get(RIGHT_PENALTY_POINT);
    }
}
...
Polygon Field::getDefenseArea(bool ourDefenseArea, double margin, bool includeOutSideField) {
    ...
    double backLineUsXCoordinate = includeOutSideField ? _field.get(LEFTMOST_X) - _field.get(BOUNDARY_WIDTH) : _field.get(LEFTMOST_X) - margin;
    double backLineThemXCoordinate = includeOutSideField ? _field.get(RIGHTMOST_X) + _field.get(BOUNDARY_WIDTH) : _field.get(RIGHTMOST_X) + margin;

    std::vector<Vector2> defenceAreaUsPoints = {{_field.get(LEFT_PENALTY_LINE).begin.x + margin, _field.get(LEFT_PENALTY_LINE).begin.y - margin},
    ...
    std::vector<Vector2> defenceAreaThemPoints = {{_field.get(RIGHT_PENALTY_LINE).begin.x - margin, _field.get(RIGHT_PENALTY_LINE).begin.y - margin},
...
Polygon Field::getGoalArea(bool ourGoal, double margin, bool hasBackMargin) {
    ...
    auto goalDepth = _field.get(GOAL_DEPTH) + marginBackside;

    if (ourGoal) {
        auto ourGoalSides = getGoalSides(true);
        std::vector<Vector2> areaUsPoints = {{ourGoalSides.first.x + margin, ourGoalSides.first.y - margin},
    ...
    std::vector<Vector2> areaThemPoints = {{theirGoalSides.first.x - margin, theirGoalSides.first.y - margin},
