...
Robot::Robot(const proto::WorldRobot &copy, Team team, unsigned char genevaState, unsigned char dribblerState, unsigned long worldNumber)
    : pidPreviousVel(Vector2()),
      distanceToBall(-1.0),
      iHaveBall(false),
...
double Robot::getDistanceToBall() { return distanceToBall; }
...
double Robot::calculateDistanceToBall(const Vector2 &ballPos) {
    ...
    if (control::ControlUtils::pointInRectangle(ballPos, leftSideOfDribbler, rightSideOfDribbler, leftSideMaxPoint, rightSideMaxPoint)) {
...
const unsigned long Robot::getLastUpdatedWorldNumber() const { return lastUpdatedWorldNumber; }

int Robot::getGenevaState() const { return genevaState; }
 ...
 */
bool Robot::genevaStateIsDifferent(int state) { return !(state == genevaState || state == 0); }
...
bool Robot::isGenevaReady() const { return world->getTime() - timeGenevaChanged >= abs(genevaState - previousGenevaState) * timeToChangeOneGenevaState; }

bool Robot::hasWorkingGeneva() const { return workingGeneva; }

unsigned char Robot::getDribblerState() const { return dribblerState; }
...
void Robot::setDribblerState(unsigned char dribbler) {
    if (dribbler < 0 || dribbler > 31) {
        ...
        if (previousDribblerState != dribblerState) {
            ...
            previousDribblerState = previousDribblerState + abs(previousDribblerState - dribblerState) / (previousDribblerState - dribblerState) * statesChanged;
        }
...
bool Robot::isDribblerReady() const { return (world::world->getTime() - timeDribblerChanged) > abs(dribblerState - previousDribblerState) * timeToChangeOneDribblerLevel; }
...
const Vector2 &Robot::getPidPreviousVel() const { return pidPreviousVel; }
...
void Robot::setHasWorkingBallSensor(bool hasWorkingBallSensor) { workingBallSensor = hasWorkingBallSensor; }

void Robot::setTimeToChangeOneGenevaState(double timeToChangeOneGenevaState) { Robot::timeToChangeOneGenevaState = timeToChangeOneGenevaState; }

bool Robot::isBatteryLow() const { return batteryLow; }

void Robot::setBatteryLow(bool batteryLow) { Robot::batteryLow = batteryLow; }

/*
 * Returns true when a feedback packet has been detected at most 2 second ago.
 */
bool Robot::hasRecentFeedback() { return (lastReceivedFeedbackMoment > world->getTime() - 1.0); }

/*
 * indicate that we got feedback at this moment.
 */
void Robot::UpdateFeedbackReceivedTime() { lastReceivedFeedbackMoment = world->getTime(); }
