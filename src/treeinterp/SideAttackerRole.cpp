 ...
 * Otherwise prepare for segfaults. When segfaults are experienced, keep in mind to check (in the debugger) whether all the nodes are added properly. Often one (or multiple) nodes
 * are nullptrs because they are added improperly and this causes segfaults in the interface
