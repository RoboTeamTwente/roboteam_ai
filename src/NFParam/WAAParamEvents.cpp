...
TargetAtTimeEvent::TargetAtTimeEvent(float value, double time, float time_constant)
    : ParamEvent(time, ParamEvent::INVALID_TIME, Anchor::START), time_constant(time_constant), target(value) {}
...
float LinearRampEvent::cumulativeValue(double start_time, double end_time, double precision) {
    ...
    return .5 * slope * (integral(end_time) - integral(start_time)) + start_value * (end_time - start_time);
}

ExponentialRampEvent::ExponentialRampEvent(float value, double time) : ParamEvent(0.0, time, Anchor::END), target(value) {}
...
float ExponentialRampEvent::cumulativeValue(double start_time, double end_time, double precision) {
    ...
    return (integral(end_time) - integral(start_time)) / std::log(base()) + start_value * (end_time - start_time);
}

ValueCurveEvent::ValueCurveEvent(const std::vector<float> &values, double start_time, double duration)
    : ParamEvent(start_time, start_time + duration, Anchor::ALL), values(values), duration(duration) {
