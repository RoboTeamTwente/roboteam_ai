...
float ParamEvent::cumulativeValue(double start_time, double end_time, double precision) {
    ...
    auto trapezoid = [this](const double &time_at, const double &time_diff) { return (this->valueAtTime(time_at + time_diff) + this->valueAtTime(time_at)) * time_diff / 2.; };
...
CustomParamEvent::CustomParamEvent(double start_time, double end_time, Anchor anchor, NF_AUDIO_PARAM_FUNCTION function)
    : ParamEvent(start_time, end_time, anchor), function(function) {}

CustomParamEvent::~CustomParamEvent() {}

float CustomParamEvent::valueAtTime(double time) { return function(time); }
