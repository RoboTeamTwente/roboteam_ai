...
namespace rtt::ai::coach {
    ...
    int PossiblePass::amountOfBlockers(const world::WorldData &world) {
        int total = 0;
        auto counter = [&](auto const &bot) { total += obstacleObstructsPath(bot->pos); };
        std::for_each(world.them.begin(), world.them.end(), counter);
        std::for_each(world.us.begin(), world.us.end(), counter);
        return total;
    }

    PossiblePass::PossiblePass(world::Robot _toBot, const Vector2 &ballPos)
            : toBot{std::move(_toBot)}, startPos{ballPos}, endPos{botReceivePos(ballPos, toBot.pos)} {
    }

    Vector2 PossiblePass::botReceivePos(const Vector2 &_startPos, const Vector2 &botPos) {
        return botPos + (_startPos - botPos).stretchToLength(Constants::CENTRE_TO_FRONT() + Constants::BALL_RADIUS());
    }

    double PossiblePass::score(const world::WorldData &world) {
        return scoreForGoalAngle(world) * penaltyForBlocks(world) * penaltyForDistance();
    }
    ...
    double PossiblePass::scoreForGoalAngle(const world::WorldData &world) {
        ...
        if (visibleParts.empty()) {
            ...
            largestOpenGoalAngle = control::ControlUtils::angleDifference(
                    control::ControlUtils::constrainAngle(angleOne),
    ...
    double PossiblePass::faceLine() const {
