...
double DefencePositionCoach::getOrientation(const Line &line) { return (line.first - line.second).angle(); }
...
std::shared_ptr<Line> DefencePositionCoach::getBlockLineSegment(const Line &openGoalSegment, const Vector2 &point, double collisionRadius, double margin) {
    if (margin == -1.0) {
...
std::shared_ptr<Vector2> DefencePositionCoach::blockOnDefenseLine(const Line &openGoalSegment, const Vector2 &point) {
...
Line DefencePositionCoach::shortenLineForDefenseArea(const Vector2 &lineStart, const Vector2 &lineEnd, double defenseMargin) {
    Line line;
    std::shared_ptr<Vector2> intersectPos = world::field->lineIntersectionWithDefenceArea(true, lineStart, lineEnd, defenseMargin);
    if (!intersectPos) {
...
std::vector<std::pair<PossiblePass, double>> DefencePositionCoach::createPassesAndDanger(const world::WorldData &world) {
...
DefenderBot DefencePositionCoach::createBlockToGoal(const PossiblePass &pass, double aggressionFactor, const Line &blockLine) {
    DefenderBot bot;
...
DefenderBot DefencePositionCoach::createBlockToGoal(const PossiblePass &pass, const Vector2 &position, const Line &blockLine) {
    DefenderBot bot;
    bot.type = botType::BLOCKTOGOAL;
    bot.blockFromID = pass.toBot.id;
...
DefenderBot DefencePositionCoach::createBlockOnLine(const PossiblePass &pass, const Vector2 &blockPos) {
    DefenderBot bot;
...
DefenderBot DefencePositionCoach::createBlockPass(PossiblePass &pass, const Vector2 &blockPoint) {
    DefenderBot bot;
...
std::shared_ptr<Vector2> DefencePositionCoach::blockOnDefenseAreaLine(const PossiblePass &pass, const world::WorldData &world) {
...
std::shared_ptr<Line> DefencePositionCoach::blockToGoalLine(const PossiblePass &pass, const world::WorldData &world) {
...
DefenderBot DefencePositionCoach::createBlockBall(const Line &blockLine) {
...
Vector2 DefencePositionCoach::findPositionForBlockBall(const Line &blockLine) {
    ...
    if (position.x > maxForwardLineX) {
        double fieldWidth = world::field->get_field().get(FIELD_WIDTH);
        Vector2 bottomLine(maxForwardLineX, -fieldWidth * 0.5);
        Vector2 topLine(maxForwardLineX, fieldWidth * 0.5);
        Vector2 intersect = control::ControlUtils::twoLineIntersection(blockLine.first, blockLine.second, bottomLine, topLine);
        return intersect;
    }
    return position;
}
double DefencePositionCoach::maxX() { return world::field->get_field().get(FIELD_LENGTH) / 10.0 * -1.0; }
world::WorldData DefencePositionCoach::getTheirAttackers(const world::WorldData &world) {
    ...
    for (auto &robot : world.them) {
        ...
        if (!world::field->pointIsInDefenceArea(robot->pos, true, 0.04) && world::field->pointIsInField(robot->pos, -0.1)) {
...
std::shared_ptr<double> DefencePositionCoach::pickNewPosition(const Line &line, const world::WorldData &world) {
...
std::shared_ptr<Vector2> DefencePositionCoach::pickNewPosition(PossiblePass pass, const world::WorldData &world) {
...
std::vector<DefenderBot> DefencePositionCoach::decidePositions(const std::vector<DefenderBot> &lockedDefenders, std::vector<int> freeRobots) {
...
std::tuple<bool, int, std::vector<int>> DefencePositionCoach::decideLockedPositions(const std::vector<rtt::ai::coach::DefenderBot> &lockedDefenders, std::vector<int> freeRobots) {
