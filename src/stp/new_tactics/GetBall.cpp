...
StpInfo GetBall::calculateInfoForSkill(StpInfo const &info) noexcept {
    ...
    Vector2 newRobotPosition = robotPosition + (ballPosition - robotPosition).stretchToLength(ballDistance - stp::control_constants::CENTER_TO_FRONT);

    bool withinFeedbackRange = ballDistance < 0.15;
    // Feedback
    if (!info.getRobot().value().hasBall() && withinFeedbackRange) {
        newRobotPosition += Vector2(0.2,0.2);
    }
