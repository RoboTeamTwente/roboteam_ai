...
uint8_t KickOffThemGameStateInvariant::metricCheck(world_new::view::WorldDataView, const world::Field *) const noexcept {
    return GameStateManager::getCurrentGameState().getStrategyName() == "kickoff_them_formation_strategy" ? stp::control_constants::FUZZY_TRUE : stp::control_constants::FUZZY_FALSE;
}
