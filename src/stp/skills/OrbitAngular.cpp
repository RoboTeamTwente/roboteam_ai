...
Status OrbitAngular::onUpdate(const StpInfo &info) noexcept {
    ...
    int speed = 1 + 3 * directionVector.toAngle().shortestAngleDiff(targetAngle); // Speed at which the robot should orbit. I made it relative to the angle so that we don't overshoot
    ...
    // Check if successful
    double errorMargin = stp::control_constants::GO_TO_POS_ANGLE_ERROR_MARGIN * M_PI_2; // Can be finetuned. smaller margin means preciser shooting but more prone to overshooting angle
