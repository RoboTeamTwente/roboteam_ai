...
vector_double TestProblem::fitness(const vector_double& dv) const {
...
std::pair<vector_double, vector_double> TestProblem::get_bounds() const {
...
const double TestProblem::rel(double x, double min, double max) const {
    if (x >= max) {
        return 1;
    }
    if (min <= x <= max) {
...
void TestProblem::updateInfoForProblem(world_new::World* problemWorld) { this->problemWorld = problemWorld; }
