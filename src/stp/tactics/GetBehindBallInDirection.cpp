...
GetBehindBallInDirection::GetBehindBallInDirection() { skills = collections::state_machine<Skill, Status, StpInfo>{skill::GoToPos(), skill::Rotate(), skill::GoToPos()}; }
...
std::optional<StpInfo> GetBehindBallInDirection::calculateInfoForSkill(StpInfo const &info) noexcept {
    ...
    if (skills.current_num() == 0) {
        newRobotPosition = ballPosition + (ballPosition - targetPosition).stretchToLength(control_constants::DISTANCE_TO_ROBOT_NEAR);
