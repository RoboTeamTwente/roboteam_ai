...
void GenericPass::onInitialize() noexcept {
    ...
    receiverPositionLeft = PositionComputations::getPosition(stpInfos["receiver_left"].getPositionToMoveTo(), field.getFrontLeftGrid().getPoints1D(), gen::GoalShootPosition, field, world);
    receiverPositionRight = PositionComputations::getPosition(stpInfos["receiver_right"].getPositionToMoveTo(), field.getFrontRightGrid().getPoints1D(), gen::GoalShootPosition, field, world);
...
void GenericPass::calculateInfoForPass(const world::ball::Ball* ball) noexcept {
    ...
    if (!passerShot) {
        ...
        receiverPositionRight =
            PositionComputations::getPosition(stpInfos["receiver_right"].getPositionToMoveTo(), field.getFrontRightGrid().getPoints1D(), gen::GoalShootPosition, field, world);
