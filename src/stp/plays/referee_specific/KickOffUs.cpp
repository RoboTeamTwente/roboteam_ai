...
void KickOffUs::calculateInfoForRoles() noexcept {
    ...
    // Kicker
    // TODO: set good position to shoot at (compute pass location)- possibly do this in the free_kick_taker role
    Vector2 passLocation = Vector2(-1.0, 1.0);
    stpInfos["free_kick_taker"].setPositionToShootAt(passLocation);
    ...
    stpInfos["free_kick_taker"].setPositionToMoveTo(Vector2(0, 0));
...
Dealer::FlagMap KickOffUs::decideRoleFlags() const noexcept {
    ...
    flagMap.insert({"free_kick_taker", {DealerFlagPriority::REQUIRED, {kickerFlag}}});
...
bool KickOffUs::shouldEndPlay() noexcept {
    if (stpInfos["receiver"].getRobot() && stpInfos["free_kick_taker"].getRobot()) {
        ...
        // True if the free_kick_taker has shot the ball, but it is now stationary (pass was too soft, was reflected, etc.)
        return ballKicked() && stpInfos["free_kick_taker"].getRobot()->get()->getDistanceToBall() >= control_constants::HAS_BALL_DISTANCE_ERROR_MARGIN * 1.5 &&
