...
void Pass::calculateInfoForRoles() noexcept {
    ...
    if (stpInfos.find("pass_receiver") != stpInfos.end()) stpInfos["pass_receiver"].setPositionMoveTo(Vector2(-2, -2));
    // Calculate Passer info
    if (stpInfos.find("passer") != stpInfos.end()) stpInfos["passer"].setPositionShootAt(Vector2(-2, -2));
    // Calculate defender1 info
    if (stpInfos.find("defender1") != stpInfos.end()) stpInfos["defender1"].setPositionMoveTo(defensivePositions[0]);
    // Calculate defender2 info
    if (stpInfos.find("defender2") != stpInfos.end()) stpInfos["defender2"].setPositionMoveTo(defensivePositions[1]);
}
