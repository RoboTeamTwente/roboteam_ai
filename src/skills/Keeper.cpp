...
Keeper::Status Keeper::onUpdate() {
    ...
    if (ball->getPos().x < 0) {
        auto attacker =
            world->getRobotClosestToPoint(ball->getPos(), THEIR_ROBOTS);
        if (attacker &&
            (ball->getPos() - attacker->pos).length() < MIN_ATTACKER_DIST) {
    ...
    if (!field->pointIsInField(
            blockPoint,
            static_cast<float>(-Constants::OUT_OF_FIELD_MARGIN()))) {
        blockPoint = goalPos;
        blockPoint.x += Constants::KEEPER_CENTREGOAL_MARGIN();
        command.set_w(0);
    } else {
        command.set_w(
            Angle((ballPos - blockPoint).angle() + M_PI_2).getAngle());
    }
    interface::Input::drawData(interface::Visual::KEEPER, {blockPoint},
                               Qt::darkYellow, robot->id,
    ...
    Vector2 velocities =
        posController.getRobotCommand(world, field, robot, blockPoint).vel;
    ...
    if (defendPos.x < field->get_field().get(OUR_GOAL_CENTER).x) {
        if (abs(defendPos.y) >= goalwidth) {
            blockPos = Vector2(goalPos.x + Constants::KEEPER_POST_MARGIN(),
                               goalwidth / 2 * signum(defendPos.y));
        } else {
            blockPos = goalPos;
        ...
        Vector2 u1 =
            (goalPos + Vector2(0.0, goalwidth * 0.5) - defendPos).normalize();
        Vector2 u2 =
            (goalPos + Vector2(0.0, -goalwidth * 0.5) - defendPos).normalize();
        ...
        std::pair<std::optional<Vector2>, std::optional<Vector2>>
            intersections =
                blockCircle.intersectionWithLine(blockLineStart, defendPos);
        ...
        if (intersections.first && intersections.second) {
            ...
            blockPos = Vector2(
                goalPos.x + Constants::KEEPER_POST_MARGIN(),
                goalwidth / 2 *
                    signum(defendPos
                               .y));  // Go stand at one of the poles depending
                                      // on the side the defendPos is on.
        }
    }

    interface::Input::drawData(interface::Visual::KEEPER, {defendPos, blockPos},
                               Qt::red, robot->id,
...
void Keeper::setGoalPosWithAttacker(RobotPtr attacker) {
    ...
    double distanceToGoal =
        ((Vector2)attacker->pos - field->get_field().get(OUR_GOAL_CENTER))
            .length();
    ...
    Vector2 i1 = control::ControlUtils::twoLineIntersection(
        attackerToBallV2 + attacker->pos, attacker->pos, goal.first,
        goal.second);
    Vector2 i2 = control::ControlUtils::twoLineIntersection(
        attackerAngleV2 + attacker->pos, attacker->pos, goal.first,
        goal.second);
    ...
    Vector2 startGoal = {-fieldMsg.get(FIELD_LENGTH) / 2,
                         -fieldMsg.get(GOAL_WIDTH) / 2};
    Vector2 endGoal = {-fieldMsg.get(FIELD_LENGTH) / 2,
                       fieldMsg.get(GOAL_WIDTH) / 2};
    if (control::ControlUtils::lineSegmentsIntersect(start, end, startGoal,
                                                     endGoal)) {
        goalPos = control::ControlUtils::twoLineIntersection(
            start, end, startGoal, endGoal);
    }
}
...
rtt::Arc Keeper::createKeeperArc() {
    ...
    double diff = rtt::ai::Constants::KEEPER_POST_MARGIN() -
                  rtt::ai::Constants::KEEPER_CENTREGOAL_MARGIN();

    double radius = diff * 0.5 +
                    goalwidth * goalwidth / (8 * diff);  // Pythagoras' theorem.
    double angle = asin(goalwidth / 2 /
                        radius);  // maximum angle (at which we hit the posts)
    Vector2 center = Vector2(
        goalPos.x + rtt::ai::Constants::KEEPER_CENTREGOAL_MARGIN() + radius, 0);
