...
bt::Node::Status BallPlacementPass::onUpdate() {
    ...
    if (!coach::g_pass.isPassed()) {
        if (coach::g_pass.isReadyToReceivePass()) {
            auto shotData = robot->getControllers().getShotController()->getRobotCommand(*field, *robot, getKicker(),
                                                                                         false,
            ...
            auto robotCommand = robot->getControllers().getNumTreePosController()->getRobotCommand(world, field, *robot, ball->get()->getPos());
