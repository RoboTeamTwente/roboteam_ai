...
std::vector<world_new::view::RobotView> StopFormation::robotsInFormation{};

StopFormation::StopFormation(std::string name, bt::Blackboard::Ptr blackboard) : Formation(std::move(name), std::move(blackboard)) {
...
std::vector<world_new::view::RobotView> StopFormation::robotsInFormationPtr() { return robotsInFormation; }

// determine the angle where the robot should point to (in position)
void StopFormation::setFinalAngle() {
    Vector2 targetToLookAtLocation = world->getBall()->get()->getPos();
    command.set_w((targetToLookAtLocation - robot->get()->getPos()).angle());
}
...
std::vector<std::vector<Vector2>> StopFormation::getStopPositions() {
    ...
    auto ourGoalCenterToBall = ball->get()->getPos() - field->getOurGoalCenter();
    auto ballToOurGoalCenter = field->getOurGoalCenter() - ball->get()->getPos();

    double distanceFromGoal;
    double distanceToBall = 1.0;

    if (ball->get()->getPos().x > 0.0) {  // if the ball is on their side
        distanceFromGoal = 4.0;
    } else {  // if the ball is on our side
        distanceFromGoal = (fabs(ball->get()->getPos().y) < 1.2) ? 1.6 : 2.2;
    }

    Vector2 closeToBallMiddle = ball->get()->getPos() + ballToOurGoalCenter.stretchToLength(distanceToBall);

    Vector2 closeToBallA = ball->get()->getPos() + ballToOurGoalCenter.stretchToLength(distanceToBall).rotate(-sin(Constants::ROBOT_RADIUS() / distanceToBall));
    Vector2 closeToBallB = ball->get()->getPos() + ballToOurGoalCenter.stretchToLength(distanceToBall).rotate(sin(Constants::ROBOT_RADIUS() / distanceToBall));
    ...
    if (ball->get()->getPos().y > goal_width) {
...
bool StopFormation::positionShouldBeAvoided(Vector2 pos) {
    return (pos.dist(ball->get()->getPos()) < 0.9 || !FieldComputations::pointIsInField(*field, pos));
}
