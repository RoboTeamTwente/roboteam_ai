...
bool FieldComputations::pointIsInDefenceArea(const ai::Field &field, const Vector2 &point, bool isOurDefenceArea, double margin, bool includeOutsideField) {
...
bool FieldComputations::pointIsInField(const ai::Field &field, const Vector2 &point, double margin) {
...
double FieldComputations::getTotalGoalAngle(const ai::Field &field, bool ourGoal, const Vector2 &point) {
    Line goal = getGoalSides(field, ourGoal);

    double angleLeft = (goal.start - point).angle();
    double angleRight = (goal.end - point).angle();

    return ai::control::NewControlUtils::angleDifference(ai::control::NewControlUtils::constrainAngle(angleLeft), ai::control::NewControlUtils::constrainAngle(angleRight));
}

/// id and ourteam are for a robot not to be taken into account.
double FieldComputations::getPercentageOfGoalVisibleFromPoint(const ai::Field &field, bool ourGoal, const Vector2 &point, const world_new::view::WorldDataView &world, int id,
                                                              bool ourTeam) {
...
std::vector<Line> FieldComputations::getBlockadesMappedToGoal(const ai::Field &field, bool ourGoal, const Vector2 &point, const world_new::view::WorldDataView &world, int id,
                                                              bool ourTeam) {
    const double robotRadius = ai::Constants::ROBOT_RADIUS() + ai::Constants::BALL_RADIUS();
    ...
    for (auto const &robot : robots) {
        if (robot->getId() == id && robot->getTeam() == (ourTeam ? Team::us : Team::them)) continue;
        ...
        if (!isRobotItself && isInPotentialBlockingZone) {
            ...
            // forward direction
            double point1val = ai::control::NewControlUtils::twoLineForwardIntersection(point, lowerSideOfRobot, lowerGoalSide, upperGoalSide);
            double point2val = ai::control::NewControlUtils::twoLineForwardIntersection(point, upperSideOfRobot, lowerGoalSide, upperGoalSide);
...
std::vector<Line> FieldComputations::getVisiblePartsOfGoal(const ai::Field &field, bool ourGoal, const Vector2 &point, const world_new::view::WorldDataView &world) {
...
double FieldComputations::getDistanceToGoal(const ai::Field &field, bool ourGoal, const Vector2 &point) {
    auto sides = getGoalSides(field, ourGoal);
    return ai::control::NewControlUtils::distanceToLineWithEnds(point, sides.start, sides.end);
}

Vector2 FieldComputations::getPenaltyPoint(const ai::Field &field, bool ourGoal) {
    if (ourGoal) {
...
std::shared_ptr<Vector2> FieldComputations::lineIntersectionWithDefenceArea(const ai::Field &field, bool ourGoal, const Vector2 &lineStart, const Vector2 &lineEnd, double margin) {
...
Polygon FieldComputations::getDefenseArea(const ai::Field &field, bool ourDefenseArea, double margin, bool includeOutSideField) {
...
Polygon FieldComputations::getGoalArea(const ai::Field &field, bool ourGoal, double margin, bool hasBackMargin) {
    ...
    if (ourGoal) {
        ...
        ai::interface::Input::drawDebugData(areaUsPoints, Qt::green, ai::interface::Drawing::LINES_CONNECTED);
    ...
    ai::interface::Input::drawDebugData(areaThemPoints, Qt::red, ai::interface::Drawing::LINES_CONNECTED);
...
Polygon FieldComputations::getFieldEdge(const ai::Field &field, double margin) {
    ...
    ai::interface::Input::drawDebugData(fieldEdge, Qt::red, ai::interface::Drawing::LINES_CONNECTED);
