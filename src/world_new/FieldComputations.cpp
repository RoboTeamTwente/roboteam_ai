...
bool FieldComputations::pointIsInDefenceArea(const ai::Field &field,
                                             const Vector2 &point,
                                             bool isOurDefenceArea,
                                             double margin,
                                             bool includeOutsideField) {
  auto defenseArea = FieldComputations::getDefenseArea(
      field, isOurDefenceArea, margin, includeOutsideField);
  return defenseArea.contains(point);
}

/// the margin is pointed inside the field!
bool FieldComputations::pointIsInField(const ai::Field &field,
                                       const Vector2 &point, double margin) {
  return (point.x <= field.getRightmostX() - margin &&
          point.x >= field.getLeftmostX() + margin &&
          point.y <= field.getTopmostY() - margin &&
          point.y >= field.getBottommostY() + margin);
}

/// returns the angle the goal points make from a point
double FieldComputations::getTotalGoalAngle(const ai::Field &field,
                                            bool ourGoal,
                                            const Vector2 &point) {
  ...
  return ai::control::ControlUtils::angleDifference(
      ai::control::ControlUtils::constrainAngle(angleLeft),
      ai::control::ControlUtils::constrainAngle(angleRight));
}

/// id and ourteam are for a robot not to be taken into account.
double FieldComputations::getPercentageOfGoalVisibleFromPoint(
    const ai::Field &field, bool ourGoal, const Vector2 &point,
    const world_new::view::WorldDataView &world, int id, bool ourTeam) {
  double goalWidth = field.getGoalWidth();
  double blockadeLength = 0;

  for (auto const &blockade :
       getBlockadesMappedToGoal(field, ourGoal, point, world, id, ourTeam)) {
...
std::vector<Line> FieldComputations::getBlockadesMappedToGoal(
    const ai::Field &field, bool ourGoal, const Vector2 &point,
    const world_new::view::WorldDataView &world, int id, bool ourTeam) {
  const double robotRadius =
      ai::Constants::ROBOT_RADIUS() + ai::Constants::BALL_RADIUS();
  ...
  for (auto const &robot : robots) {
    if (robot->getId() == id &&
        robot->getTeam() == (ourTeam ? Team::us : Team::them))
      continue;

    double lenToBot = (point - robot->getPos()).length();

    // discard already all robots that are not at all between the goal and
    // point, or if a robot is standing on this point
    bool isRobotItself = lenToBot <= robotRadius;
    bool isInPotentialBlockingZone =
        ourGoal ? robot->getPos().x < point.x + robotRadius
                : robot->getPos().x > point.x - robotRadius;
    ...
    if (!isRobotItself && isInPotentialBlockingZone) {
      ...
      Vector2 lowerSideOfRobot =
          point + Vector2(length, 0).rotate(
                      (Vector2(robot->getPos()) - point).angle() - theta);
      Vector2 upperSideOfRobot =
          point + Vector2(length, 0).rotate(
                      (Vector2(robot->getPos()) - point).angle() + theta);

      // map points onto goal line

      // the forwardIntersection returns a double which is the scale of the
      // vector projection this returns -1.0 if there is no intersections in the
      // forward direction
      double point1val = ai::control::ControlUtils::twoLineForwardIntersection(
          point, lowerSideOfRobot, lowerGoalSide, upperGoalSide);
      double point2val = ai::control::ControlUtils::twoLineForwardIntersection(
          point, upperSideOfRobot, lowerGoalSide, upperGoalSide);

      // Here is how we calculate the actual intersections using the above
      // values
      ...
      // these following 2 cases are identical in logic but mirrored; one point
      // hits the backline, other does not. in that case, we pick the
      // appropriate goalPost which would be right for the obstacle as new Point
      // and check if this interval is valid
      ...
      // check if both points are below or above the goal (this invalidates it
      // again)
      if (validObstacle) {
        bool bothPointsBelowGoal =
            point1.y <= lowerGoalSide.y && point2.y <= lowerGoalSide.y;
        bool bothPointAboveGoal =
            point1.y >= upperGoalSide.y && point2.y >= upperGoalSide.y;
...
std::vector<Line> FieldComputations::getVisiblePartsOfGoal(
    const ai::Field &field, bool ourGoal, const Vector2 &point,
    const world_new::view::WorldDataView &world) {
  ...
  // everytime we add a vector from the lowest goalside to the lowest part of
  // the obstacle we remember the upper part of the obstacle That upper part is
  // stored as the lowerhook again: and we can repeat the process
  for (auto const &blockade : blockades) {
    auto lowerbound = fmin(blockade.start.y, blockade.end.y);
    // if the lowerbound is the same as the lower hook then the visible part has
    // a length of 0 and we don't care about it originally used to be != but
    // floating point errors are tears.
    if (fabs(lowerbound - lowerHook.y) > 0.000001) {
      visibleParts.emplace_back(
          Line(lowerHook, Vector2(blockade.start.x, lowerbound)));
    }
    auto upperbound = fmax(blockade.start.y, blockade.end.y);
    lowerHook = Vector2(blockade.start.x, upperbound);
  }
  // if the last lowerhook is the same as the upper goal side then the visible
  // part has a length of 0 and we don't care about it
  if (lowerHook != upper) {
...
/**
 * if two blockades intersect (in this case, overlap), we take the beginning of
 * the first obstacle and the end of the second obstacle, and put them back in
 * the front of the obstacles vector. The second element gets erased. if they
 * don't intersect, try the next two obstacles. repeat until no overlaps are
 * left.
 */
std::vector<Line>
FieldComputations::mergeBlockades(std::vector<Line> blockades) {
  ...
  while (blockades.size() > (iterator + 1)) {
    if (blockades.at(iterator).end.y >= blockades.at(iterator + 1).start.y) {
      ...
      auto upperbound = fmax(blockades.at(iterator).end.y,
                             blockades.at(iterator + 1).start.y);

      // construct a new vector from the lowest to highest blockade value
      auto newBlockade =
          Line(blockades.at(iterator).start,
               Vector2(blockades.at(iterator).start.x, upperbound));
...
/// Returns the sides of the goal. The first vector is the the lower side and
/// the second is the upper side.
Line FieldComputations::getGoalSides(const ai::Field &field, bool ourGoal) {
  if (ourGoal) {
...
double FieldComputations::getDistanceToGoal(const ai::Field &field,
                                            bool ourGoal,
                                            const Vector2 &point) {
  auto sides = getGoalSides(field, ourGoal);
  return ai::control::ControlUtils::distanceToLineWithEnds(point, sides.start,
                                                           sides.end);
}

Vector2 FieldComputations::getPenaltyPoint(const ai::Field &field,
                                           bool ourGoal) {
  if (ourGoal) {
...
std::shared_ptr<Vector2> FieldComputations::lineIntersectionWithDefenceArea(
    const ai::Field &field, bool ourGoal, const Vector2 &lineStart,
...
Polygon FieldComputations::getDefenseArea(const ai::Field &field,
                                          bool ourDefenseArea, double margin,
                                          bool includeOutSideField) {
  double backLineUsXCoordinate =
      includeOutSideField ? field.getLeftmostX() - field.getBoundaryWidth()
                          : field.getLeftmostX() - margin;
  double backLineThemXCoordinate =
      includeOutSideField ? field.getRightmostX() + field.getBoundaryWidth()
                          : field.getRightmostX() + margin;

  std::vector<Vector2> defenceAreaUsPoints = {
      {field.getLeftPenaltyLine().begin.x + margin,
       field.getLeftPenaltyLine().begin.y - margin},
      {field.getLeftPenaltyLine().end.x + margin,
       field.getLeftPenaltyLine().end.y + margin},
      {backLineUsXCoordinate, field.getLeftPenaltyLine().end.y + margin},
      {backLineUsXCoordinate, field.getLeftPenaltyLine().begin.y - margin}};

  ai::interface::Input::drawDebugData(defenceAreaUsPoints);
  Polygon defenceAreaUs(defenceAreaUsPoints);

  std::vector<Vector2> defenceAreaThemPoints = {
      {field.getRightPenaltyLine().begin.x - margin,
       field.getRightPenaltyLine().begin.y - margin},
      {field.getRightPenaltyLine().end.x - margin,
       field.getRightPenaltyLine().end.y + margin},
...
Polygon FieldComputations::getGoalArea(const ai::Field &field, bool ourGoal,
                                       double margin, bool hasBackMargin) {
  ...
  if (ourGoal) {
    ...
    ai::interface::Input::drawDebugData(
        areaUsPoints, Qt::green, ai::interface::Drawing::LINES_CONNECTED);
  ...
  ai::interface::Input::drawDebugData(areaThemPoints, Qt::red,
                                      ai::interface::Drawing::LINES_CONNECTED);

  return Polygon(areaThemPoints);
}

Polygon FieldComputations::getFieldEdge(const ai::Field &field, double margin) {
  ...
  ai::interface::Input::drawDebugData(fieldEdge, Qt::red,
                                      ai::interface::Drawing::LINES_CONNECTED);
