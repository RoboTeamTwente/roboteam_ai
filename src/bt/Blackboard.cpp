...
void Blackboard::setString(std::string key, std::string value) { strings[key] = std::move(value); }
...
void Blackboard::setVector2(std::string key, rtt::Vector2 value) { vectors[key] = value; }
bool Blackboard::hasVector2(std::string key) const { return vectors.find(key) != vectors.end(); }
