...
void DribbleForwards::updateForwardsProgress() {
    ...
    switch (forwardsProgress) {
        ...
        case APPROACHING: {
            ...
            if (robot.hasBall()) {
                forwardsDribbleLine = {robot->getPos(), finalTargetPos};
...
RobotCommand DribbleForwards::sendApproachCommand() {
    ...
    command.dribbler = 31;
    command.vel = (robot->getPos() - ball->getPos()).stretchToLength(maxVel);
...
RobotCommand DribbleForwards::sendDribbleForwardsCommand() {
    ...
    double maxDist = errorMargin * 8 + std::fmin(1.0, 2 * (robot->getPos() - finalTargetPos).length());

    if (control::ControlUtils::distanceToLine(robot->getPos(), forwardsDribbleLine.first, forwardsDribbleLine.second) > maxDist) {
        forwardsProgress = TURNING;
    }

    Angle robotAngleTowardsLine = (finalTargetPos - robot->getPos()).toAngle() - (forwardsDribbleLine.second - forwardsDribbleLine.first).toAngle();

    double compensationFactor = std::fmax(4.0, 8.0 * (ramp - waitingTicks) / ramp);
    Vector2 compensation = (robot->getAngle() + M_PI_2).toVector2(std::min(robotAngleTowardsLine * compensationFactor, 0.05));
    ...
    interface::Input::drawData(interface::Visual::BALL_HANDLING, {forwardsDribbleLine.first, forwardsDribbleLine.second}, Qt::white, robot->getId(),
                               interface::Drawing::LINES_CONNECTED);

    // limit velocity close to the target
    double distanceToTarget = (finalTargetPos - robot->getPos()).length();
