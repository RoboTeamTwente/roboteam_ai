...
RobotCommand BallHandlePosControl::handleBall(const Vector2 &targetBallPos, TravelStrategy travelStrategy, bool shouldGoToBall, bool ballIsFarFromTarget) {
    ...
    RobotCommand BallHandlePosControl::handleBall(const Vector2 &targetBallPos, TravelStrategy travelStrategy, bool shouldGoToBall, world_new::view::RobotView _robot, bool ballIsFarFromTarget) {
        if (shouldGoToBall) {
            status = GET_BALL;
...
RobotCommand BallHandlePosControl::interceptMovingBall(const Vector2 &projectionPosition, double ballToProjectionDistance, const Angle &robotAngleTowardsBallVel) {
    ...
    RobotCommand BallHandlePosControl::interceptMovingBall(const Vector2 &projectionPosition, double ballToProjectionDistance, const Angle &robotAngleTowardsBallVel, world_new::view::RobotView _robot) {
        Vector2 numTreesTarget = projectionPosition;
        RobotCommand robotCommand;
...
bool BallHandlePosControl::isCrashingOutsideField(const LineSegment &driveLine) {
    ...
    bool BallHandlePosControl::isCrashingOutsideField(const LineSegment &driveLine, world_new::view::RobotView _robot) {
        if (!FieldComputations::pointIsInField(*world_new::World::instance()->getField(), driveLine.end)) {
            interface::Input::drawData(interface::Visual::BALL_HANDLING, {control::ControlUtils::projectPositionToWithinField(*world_new::World::instance()->getField(), driveLine.end)}, Qt::red, _robot->getId(),
                                       interface::Drawing::CIRCLES, 24, 24, 12);
