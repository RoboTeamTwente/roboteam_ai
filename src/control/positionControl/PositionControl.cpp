...
rtt::BB::CommandCollision PositionControl::computeAndTrackPathBBT(const rtt::world::World *world, const rtt::world::Field &field, int robotId, Vector2 currentPosition,
    ...
    if (false || (!computedPathsPosVel.contains(robotId)) ||
                  (!computedPaths[robotId].empty() && (targetPosition - computedPaths[robotId][computedPaths[robotId].size() - 1]).length() >
stp::control_constants::GO_TO_POS_ERROR_MARGIN) || worldObjects.getFirstCollisionDiscretized(world, field, computedPathsPosVel[robotId], computedPaths, robotId,
timeStep).has_value() ) {
