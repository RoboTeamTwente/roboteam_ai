...
RobotCommand NumTreePosControl::getRobotCommand(world_new::view::WorldDataView *world, const world::Field *field, const world_new::view::RobotView &robotPtr, const Vector2 &targetPos) {
/*    this->world = world;
    this->field = field;
    this->robot = robotPtr;*/
...
RobotCommand NumTreePosControl::getRobotCommand(world_new::view::WorldDataView *world, const world::Field *field, const world_new::view::RobotView &robotPtr, const Vector2 &targetPos,
    ...
    bool nicePath = true;
    bool recalculate = doRecalculatePath(world, targetPos);
    if (recalculate) {
...
void NumTreePosControl::tracePath(world_new::view::WorldDataView *world) {
    ...
    root->pos = robot->getPos();
    root->vel = robot->getVel();
    root->acc = Vector2();
    root->t = 0;
    root->collisions = 0;
    pathQueue.push(root);
    ...
    while (!pathQueue.empty()) {
        ...
        if ((now - start).count() * 1000 > MAX_CALCULATION_TIME) {
            if (InterfaceValues::showDebugNumTreeInfo()) {
                std::cout << "ROBOT " << robot->getId() << ": Tick took too long!" << std::endl;
            }
        ...
        while (true) {
            ...
            Collision collision = getCollision(world, point, DEFAULT_ROBOT_COLLISION_RADIUS);
    ...
    if (InterfaceValues::showDebugNumTreeInfo()) {
        std::cout << "ROBOT " << robot->getId() << ": reached end of while loop, no path found" << std::endl;
    }
    path = {};
}
...
Collision NumTreePosControl::getCollision(world_new::view::WorldDataView *world, const PathPointer &point, double collisionRadius) {
    this->world = world;
    ...
    auto allRobots = this->world->getRobotsNonOwning();
...
Collision NumTreePosControl::getRobotCollision(const PathPointer &point, const std::vector<world_new::view::RobotView> &robots, double distance) {
    ...
    for (auto &r : robots) {
        ...
        if(currentRobotCollision) {
            if (r->getId() == currentRobotCollision->getId() &&
                r->getTeam() == currentRobotCollision->getTeam())
                return collision;
        }

        if(currentFinalTargetCollision) {
            // don't look at the final target if the robot is colliding
            if (r->getId() == currentFinalTargetCollision->getId() &&
                r->getTeam() == currentFinalTargetCollision->getTeam())
                return collision;
        }
...
Collision NumTreePosControl::getBallCollision(const PathPointer &point, const world_new::view::BallView &ball) {
    Collision collision = {};
    if(currentCollisionWithRobot.getCollisionBall()) {
        if (currentCollisionWithRobot.getCollisionBall()->isVisible()) return collision;
    }

    if(currentCollisionWithFinalTarget.getCollisionBall()) {
...
Collision NumTreePosControl::getFieldCollision(const PathPointer &point) {
    Collision collision = {};
    bool canMoveOutOfField =
        currentCollisionWithRobot.getCollisionFieldPos() != Vector2() || currentCollisionWithFinalTarget.getCollisionFieldPos() != Vector2() || getCanMoveOutOfField(robot->getId());
...
Collision NumTreePosControl::getGoalCollision(const NumTreePosControl::PathPointer &point) {
    ...
    bool collidesWithOurGoal = FieldComputations::getGoalArea(*field, true, Constants::ROBOT_RADIUS(), true).contains(point->pos);
    bool collidesWithTheirGoal = FieldComputations::getGoalArea(*field, false, Constants::ROBOT_RADIUS(), true).contains(point->pos);
...
bool NumTreePosControl::doRecalculatePath(world_new::view::WorldDataView *world, const Vector2 &targetPos) {
    ...
    if (checkCurrentRobotCollision(world)) return true;

    return checkIfRobotWillCollideFollowingThisPath(world);
}

bool NumTreePosControl::checkCurrentRobotCollision(world_new::view::WorldDataView *world) {
    ...
    realRobotPoint->pos = robot->getPos();
    realRobotPoint->vel = robot->getVel();
    realRobotPoint->t = 0;
    currentCollisionWithRobot = getCollision(world, realRobotPoint, DEFAULT_ROBOT_COLLISION_RADIUS);
    ...
    currentCollisionWithFinalTarget = getCollision(world, finalTargetPoint, DEFAULT_ROBOT_COLLISION_RADIUS);
    ...
    if (collision) {
        if (InterfaceValues::showDebugNumTreeInfo()) {
            ...
            std::cout << "ROBOT " << robot->getId() << ": is too close to " << s << "-> trying to make a path anyways <-" << std::endl;
        }
        path.clear();
        pathHasRobotCollision = false;
        return true;
...
bool NumTreePosControl::checkEmptyPath() {
    if (path.empty()) {
        if (InterfaceValues::showFullDebugNumTreeInfo()) {
            std::cout << "ROBOT " << robot->getId() << ": no path, recalculating" << std::endl;
        }
        return true;
    }
    return false;
...
bool NumTreePosControl::checkIfTargetMoved(double maxTargetDeviation, const Vector2 &targetPos) {
    ...
    if ((finalTargetPos - targetPos).length2() > maxTargetDeviation * maxTargetDeviation) {
        if (InterfaceValues::showFullDebugNumTreeInfo()) {
            std::cout << "ROBOT " << robot->getId() << ": target moved too much, recalculating" << std::endl;
        }
        return true;
    }
    return false;
...
bool NumTreePosControl::checkIfAtEndOfPath(double maxTargetDeviation, const Vector2 &targetPos) {
    ...
    if (path.size() < static_cast<unsigned int>(1.01 + 1.0 / DT)) {
        if ((path[path.size() - 1].pos - targetPos).length() > maxTargetDeviation) {
            if (InterfaceValues::showFullDebugNumTreeInfo()) {
                std::cout << "ROBOT " << robot->getId() << ": reached end of path segment, recalculating" << std::endl;
            }
            return true;
        }
    }
...
bool NumTreePosControl::checkIfTooFarFromCurrentPath(double maxTargetDeviation, const Vector2 &vector2) {
    ...
    if (sqrt(distanceSquared) > maxTargetDeviation) {
        if (InterfaceValues::showDebugNumTreeInfo()) {
            std::cout << "ROBOT " << robot->getId() << ": is too far from current path, recalculating" << std::endl;
        }
        return true;
    }
...
bool NumTreePosControl::checkIfRobotWillCollideFollowingThisPath(world_new::view::WorldDataView *world) {
    // check if there is a collision for any of the upcoming points in the path
    for (const auto &pathPoint : path) {
        if (getCollision(world, std::make_shared<PathPoint>(pathPoint), 0.8 * DEFAULT_ROBOT_COLLISION_RADIUS).isCollision) {
            if (InterfaceValues::showDebugNumTreeInfo()) {
