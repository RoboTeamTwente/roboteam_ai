...
HasClearShot::Status HasClearShot::onUpdate() {
    if ((Vector2(ball->getPos()) - world::field->get_field().get(THEIR_GOAL_CENTER)).length() < FORCED_SHOOTING_DISTANCE) {
    ...
    if ((Vector2(ball->getPos()) - world::field->get_field().get(THEIR_GOAL_CENTER)).length() > MAX_SHOOTING_DISTANCE) {
    ...
    bool hasClearShot = world::field->getPercentageOfGoalVisibleFromPoint(false, ball->getPos(), world, robot->id, true)
            > minViewAtGoal * 100;
