...
// This test runs 5000 worldstates and sees if the changes get propagated properly.
TEST(WorldTest, unstable_worldstate) {
    roboteam_msgs::World worldMsg;
    roboteam_msgs::GeometryFieldSize field;
    field.field_width = 9;
    field.field_length = 12;

    // run tests for 5000 different worldstates
    for (int i = 0; i < 5000; i++) {

        int amountOfRobotsUs = (int) testhelpers::WorldHelper::getRandomValue(1, 8);
        int amountOfRobotsThem = (int) testhelpers::WorldHelper::getRandomValue(1, 8);

        // create a world message with 8v8 where we have the ball
        auto pair = testhelpers::WorldHelper::getWorldMsgWhereRobotHasBall(amountOfRobotsUs, amountOfRobotsThem, true,
                                                                           field);
        worldMsg = pair.first;
        int robotWithBallId = pair.second;
        ...
        // The proper robot has the ball
        ASSERT_EQ(world->whichRobotHasBall()->id, robotWithBallId);
        ASSERT_TRUE(world->robotHasBall(robotWithBallId, true));
        ASSERT_EQ(world->getRobotClosestToBall().id, robotWithBallId);

        // Proper amount robots
        ASSERT_EQ(world->getUs().size(), amountOfRobotsUs);
        ASSERT_EQ(world->getThem().size(), amountOfRobotsThem);
        ASSERT_EQ(world->getAllRobots().size(), amountOfRobotsUs + amountOfRobotsThem);

        // the teams are set correctly
        for (auto const &bot : world->getUs()) { ASSERT_EQ(bot.team, rtt::ai::world::Robot::Team::us); }
        for (auto const &bot : world->getThem()) { ASSERT_EQ(bot.team, rtt::ai::world::Robot::Team::them); }
    }
}