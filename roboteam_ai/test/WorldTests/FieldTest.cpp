...
TEST(FieldTest, it_gets_points_in_defence_area) {
    ...
    // generate 100 random positions in our defense area
    for (int i = 0; i < 100; i ++) {
        auto x = testhelpers::WorldHelper::getRandomValue(- 6, - 4);
        auto y = testhelpers::WorldHelper::getRandomValue(0, 8);

        // all points should be in our defense area
        rtt::ai::world::field->set_field(field);
        bool inOurDefenseArea = rtt::ai::world::field->pointIsInDefenseArea(rtt::Vector2(x, y), true, 0.0);
        EXPECT_TRUE(inOurDefenseArea);

        // the points should not be in their defense area
        bool inTheirDefenseArea = rtt::ai::world::field->pointIsInDefenseArea(rtt::Vector2(x, y), false, 0.0);
        EXPECT_FALSE(inTheirDefenseArea);
    }

    // generate 100 random positions outside our defense area
    for (int i = 0; i < 100; i ++) {
        auto x = testhelpers::WorldHelper::getRandomValue(- 4, 4);
        auto y = testhelpers::WorldHelper::getRandomValue(0, 8);
        rtt::ai::world::field->set_field(field);
        bool inDefenseArea = rtt::ai::world::field->pointIsInDefenseArea(rtt::Vector2(x, y), true, 0.0);
        EXPECT_FALSE(inDefenseArea);
    }
}
...
TEST(FieldTest, it_calculates_obstacles) {
    ...
    EXPECT_FLOAT_EQ(rtt::ai::world::field->getPercentageOfGoalVisibleFromPoint(true, {0, 0}),
            round(100 - 200*(rtt::ai::Constants::ROBOT_RADIUS() + rtt::ai::Constants::BALL_RADIUS())));
    ...
    EXPECT_FLOAT_EQ(rtt::ai::world::field->getPercentageOfGoalVisibleFromPoint(false, {0, 0}),
            round(100 - 400*(rtt::ai::Constants::ROBOT_RADIUS() + rtt::ai::Constants::BALL_RADIUS())));
