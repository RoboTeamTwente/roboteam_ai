...
TEST(CoachTest, offensive_coach_test) {
    ...
    std::vector<Vector2> offensivePositions = rtt::ai::coach::g_offensiveCoach.updateOffensivePositions();
    ...
    while (counter < 10) {
        newOffensivePositions = rtt::ai::coach::g_offensiveCoach.updateOffensivePositions();
        counter++;
    }
