...
TEST(IOTest, strategyIOManager) {
  ...
  // publishing
  ros::Subscriber roleDirectiveSub = nh.subscribe<roboteam_msgs::RoleDirective>(rtt::TOPIC_ROLE_DIRECTIVE, 0, &roleDirectiveCallback);
...
TEST(IOTest, roleIOManager) {
  ...
  // publishing
  ros::Subscriber robotCommandSub = nh.subscribe<roboteam_msgs::RobotCommand>(rtt::TOPIC_COMMANDS, 0, &robotCommandCallback);
