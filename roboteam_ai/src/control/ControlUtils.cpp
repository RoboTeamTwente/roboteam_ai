...
double ControlUtils::calculateAngularVelocity(double robotAngle, double targetAngle) {
    ...
    return direction*(std::pow(rotFactor, angleDiff - 1) * Constants::getDouble("MAX_ANGULAR_VELOCITY") - 1/rotFactor);
}
...
bool ControlUtils::hasClearVision(int fromID, int towardsID, roboteam_msgs::World world, int safelyness) {
    double minDistance = Constants::getDouble("ROBOT_RADIUS")*(3*safelyness); // TODO: calibrate Rolf approved
    Vector2 fromPos;
    Vector2 towardsPos;
...
rtt::Arc ControlUtils::createKeeperArc() {
    ...
    double diff = Constants::getDouble("KEEPER_POST_MARGIN") -  Constants::getDouble("KEEPER_CENTREGOAL_MARGIN");
    ...
    Vector2 center = Vector2(goalPos.x +  Constants::getDouble("KEEPER_CENTREGOAL_MARGIN") + radius, 0);
    if (diff > 0) {
...
Vector2 ControlUtils::VelocityLimiter(Vector2 vel) {
    if (vel.length() > Constants::getDouble("MAX_VEL")) {
        vel = vel.stretchToLength(Constants::getDouble("MAX_VEL"));
        return vel;
    }
    else return vel;
}
