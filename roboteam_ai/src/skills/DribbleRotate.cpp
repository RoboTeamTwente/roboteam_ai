...
void DribbleRotate::checkProgression() {
    ...
    if (!robotHasBall(Constants::getDouble("MAX_BALL_BOUNCE_RANGE"))){ // change to !botHassball() alter
...
void DribbleRotate::onInitialize() {
    if (properties->hasDouble("maxVel")) {
        ...
        maxSpeed = Constants::getDouble("DRIBBLE_ROTATE_MAX_SPEED");
    }
    ...
    incrementAngle= maxSpeed/Constants::getDouble("tickRate");
    currentProgression=ROTATING;
    currentTick=0;
    extraTick= static_cast<int>(Constants::getDouble("DRIBBLE_ROTATE_WAIT_TIME")*Constants::getDouble("tickRate"));
    dir=Control::rotateDirection(startAngle,targetAngle);
    maxTick=(int)floor(Control::angleDifference(startAngle,targetAngle)/maxSpeed*Constants::getDouble("tickRate"));
    if(!ball->visible){
        auto world=World::get_world();
        Vector2 ballPos=Vector2(robot->pos)+Vector2(Constants::getDouble("ROBOT_RADIUS")+Constants::getDouble("BALL_RADIUS"),0).rotate(robot->angle);
        world.ball.visible=true;
        world.ball.pos=ballPos;
        World::set_world(world);
    }
    if (!robotHasBall(Constants::getDouble("MAX_BALL_RANGE"))){
        std::cout<<"Robot does not have ball in dribbleRotateInitialize"<<std::endl;
        std::cout<< "Distance"<<(Vector2(robot->pos)-Vector2(ball->pos)).length()-Constants::getDouble("ROBOT_RADIUS")<<" Max distance:"<<Constants::getDouble("MAX_BALL_RANGE")<<std::endl;
        currentProgression=FAIL;
