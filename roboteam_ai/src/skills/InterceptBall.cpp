...
void InterceptBall::onInitialize() {
    ...
    tickCount = 0;
    maxTicks = static_cast<int>(floor(Constants::getInt("MAX_INTERCEPT_TIME")*Constants::getInt("tickRate")));
    ballStartPos = ball->pos;
    ballStartVel = ball->vel;
    ballEndPos = ballStartPos + ballStartVel * Constants::getInt("MAX_INTERCEPT_TIME");
    ...
    pid.setPID(5.7,1.7,0.0,1.0/Constants::getInt("tickRate")); //TODO:magic numbers galore, from the old team. Move to new control library?
    finePid.setPID(5.7,1.7,0.0, 1.0/Constants::getInt("tickRate"));
}
InterceptBall::Status InterceptBall::onUpdate() {
    ...
    if (keeper) {
        interceptPos = computeInterceptPoint(ball->pos,
                Vector2(ball->pos) + Vector2(ball->vel)*Constants::getInt("MAX_INTERCEPT_TIME"));
    }
    deltaPos = interceptPos - robot->pos;
    checkProgression();
    //interface
    ...
    displayColorData.emplace_back(std::make_pair(Vector2(ball->pos)+ Vector2(ball->vel)*Constants::getInt("MAX_INTERCEPT_TIME"), Qt::green));
...
bool InterceptBall::missedBall(Vector2 startBall, Vector2 endBall, Vector2 ballVel) {
    ...
    double angleDev = tan(Constants::getDouble("ROBOT_RADIUS")/interceptDist);
    double rectHalfLength = atan(angleDev)
            *(interceptDist + Constants::getDouble("ROBOT_RADIUS"));// Half of the rectangle covered by the robot behind the robot.
    ...
    Vector2 startCentre = startBall + ballVel.stretchToLength(interceptDist + Constants::getDouble("ROBOT_RADIUS"));
...
bool InterceptBall::ballDeflected() {
    ...
    if (abs(control::ControlUtils::constrainAngle(Vector2(ball->vel).angle() - ballStartVel.angle()))
            > Constants::getDouble("BALL_DEFLECTION_ANGLE")) {
        return true;
    }
...
bool InterceptBall::ballToGoal() {
    ...
    double margin = Constants::getDouble("BALL_TO_GOAL_MARGIN");
    Vector2 lowerPost = goalCentre + Vector2(0.0, - (goalWidth + margin));
    Vector2 upperPost = goalCentre + Vector2(0.0, goalWidth + margin);
    Vector2 ballPos = ball->pos;
    Vector2 ballPredPos = Vector2(ball->pos) + Vector2(ball->vel)*Constants::getDouble("MAX_INTERCEPT_TIME");
