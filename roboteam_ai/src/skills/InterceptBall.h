...
class InterceptBall : public Skill {
    ...
    // if the ball is shot next to the goal within this margin we still try to intercept
        enum Progression {
          INTERCEPTING,
          INPOSITION,
          BALLDEFLECTED,
    ...
    virtual Vector2 computeInterceptPoint(Vector2 startBall, Vector2 endBall);

    const double GOAL_MARGIN = Constants::BALL_RADIUS() + 0.05;
