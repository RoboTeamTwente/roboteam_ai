...
Vector2 Coach::getBallPlacementBeforePos(Vector2 ballPos){
    ...
    Vector2 targetPos=ballPos + (PlacePos - ballPos).stretchToLength(Constants::getDouble("BP_MOVE_TOWARDS_DIST"));
    return targetPos;
}
Vector2 Coach::getBallPlacementAfterPos(double RobotAngle){
    Vector2 targetPos=interface::InterfaceValues::getBallPlacementTarget() + Vector2(Constants::getDouble("BP_MOVE_BACK_DIST"),0).rotate(RobotAngle+M_PI);
    return targetPos;
}
