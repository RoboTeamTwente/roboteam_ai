    ...
    if (rtt::ai::World::didReceiveFirstWorld) {
        ...
        if (showPath) {
            for (auto robot : selectedRobots) {
                ...
                drawDataPoints(painter, Drawer::getKeeperPoints(robot.id), Constants::getInt("KEEPER_HELP_DRAW_SIZE"));
...
void Visualizer::calculateFieldSizeFactor() {
    ...
    fieldmargin = static_cast<int>(Constants::getInt("WINDOW_FIELD_MARGIN") + field.boundary_width);
...
void Visualizer::drawBackground(QPainter &painter) {
    painter.setBrush(Constants::getColor("FIELD_COLOR"));
...
void Visualizer::drawBall(QPainter &painter) {
    ...
    painter.setBrush(Constants::getColor("BALL_COLOR")); // fill
    painter.setPen(Qt::NoPen); // stroke
    painter.drawEllipse(qballPosition, Constants::getInt("BALL_DRAWING_SIZE"), Constants::getInt("BALL_DRAWING_SIZE"));
}
...
void Visualizer::drawRobot(QPainter &painter, roboteam_msgs::WorldRobot robot, bool ourTeam) {
    ...
    if (ourTeam) {
        ...
        robotColor =weAreYellow ? Constants::getColor("ROBOT_COLOR_YELLOW") : Constants::getColor("ROBOT_COLOR_BLUE");
    } else {
        // the enemy robot should have the inverse of our_color
        robotColor = weAreYellow ? Constants::getColor("ROBOT_COLOR_BLUE") : Constants::getColor("ROBOT_COLOR_YELLOW");
    }
    ...
    if (showTactics && ourTeam) {
        painter.setPen(Constants::getColor("TEXT_COLOR"));
    ...
    // draw the robots
    QColor color = (robotIsSelected(robot) && ourTeam) ? Constants::getColor("SELECTED_ROBOT_COLOR") : robotColor;
    painter.setBrush(color);
    painter.setPen(Qt::transparent);
    painter.drawEllipse(qrobotPosition, Constants::getInt("ROBOT_DRAWING_SIZE"), Constants::getInt("ROBOT_DRAWING_SIZE"));
...
void Visualizer::drawTacticColorForRobot(QPainter &painter, roboteam_msgs::WorldRobot robot) {
    ...
    if (! tacticExists) {
        QColor TacticColors[] = {Constants::getColor("TACTIC_1"),
                              Constants::getColor("TACTIC_2"),
                              Constants::getColor("TACTIC_3"),
                              Constants::getColor("TACTIC_4"),
                              Constants::getColor("TACTIC_5")};

        QColor newColor = TacticColors[tacticCount];
        tacticCount = (tacticCount + 1)%sizeof(TacticColors);
    ...
    painter.setPen(Qt::transparent);
    painter.setBrush(c);
    painter.drawEllipse(qrobotPosition, Constants::getInt("TACTIC_COLOR_DRAWING_SIZE"), Constants::getInt("TACTIC_COLOR_DRAWING_SIZE"));
}
...
void Visualizer::drawIntercept(QPainter &painter, std::vector<std::pair<rtt::Vector2, QColor>> points) {
    if (! points.empty()) {
        for (int j = 0; j < Constants::getInt("INTERCEPT_DRAW_VECTOR_SIZE"); ++j) {
            ...
            if (j < 1) {
                ...
                painter.drawEllipse(PointZ.x, PointZ.y, Constants::getInt("KEEPER_HELP_DRAW_SIZE"),
                        Constants::getInt("KEEPER_HELP_DRAW_SIZE"));
            }
            else {
                painter.setPen(points[j].second);
