...
TEST(ShotControllerTest, getshotdata_test) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        shotdata = shotController.getRobotCommand(*robot, testhelpers::WorldHelper::getRandomFieldPosition(field), true, BallSpeed::MAX_SPEED, true, ShotPrecision::HIGH);
        EXPECT_FALSE(shotdata.kicker);
        EXPECT_FLOAT_EQ(shotdata.geneva, 3);
    }
    ...
    for (int i = 0; i < 100; i++) {
        ...
        shotdata = shotController.getRobotCommand(*robot, testhelpers::WorldHelper::getRandomFieldPosition(field), false, BallSpeed::MAX_SPEED, true, ShotPrecision::HIGH);
