...
TEST(RobotCommandTest, it_converts_to_ros_command) {
    for (int i = 0; i < 100; i++) {
        ...
        EXPECT_FLOAT_EQ(cmd.vel().x(), rc.vel.x);
        EXPECT_FLOAT_EQ(cmd.vel().y(), rc.vel.y);
        EXPECT_FLOAT_EQ(cmd.w(), rc.angle);
        EXPECT_EQ(cmd.kicker(), rc.kicker);
        EXPECT_EQ(cmd.chip_kick_forced(), rc.kickerForced);
        EXPECT_EQ(cmd.chipper(), rc.chipper);
        EXPECT_EQ(cmd.chip_kick_forced(), rc.chipperForced);
        EXPECT_FLOAT_EQ(cmd.chip_kick_vel(), rc.kickerVel);
