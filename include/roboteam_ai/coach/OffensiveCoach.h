...
namespace rtt::ai::coach {
    ...
    public:
        ...
        constexpr static double SEARCH_GRID_ROBOT_POSITIONS = 0.055;
        constexpr static double CLOSE_TO_GOAL_DISTANCE = 0.37;
        constexpr static double FURTHER_FROM_GOAL_DISTANCE = 6.0 * CLOSE_TO_GOAL_DISTANCE;
    ...
    private:
        OffensivePosition findBestOffensivePosition(const std::vector<Vector2> &positions,
                                                    const OffensivePosition &currentBestScore,
                                                    const Vector2 &zoneLocation);

        const std::pair<Vector2, Vector2> &
        getLongestSegment(const std::vector<std::pair<Vector2, Vector2>> &openSegments);
        ...
        std::unordered_map<int, int> sideAttackers; // Map from robot ids to zones
