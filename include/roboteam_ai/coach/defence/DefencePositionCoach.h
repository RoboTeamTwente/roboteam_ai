...
class DefencePositionCoach {
   ...
   public:
    ...
    std::shared_ptr<Vector2> blockOnDefenseAreaLine(const PossiblePass &pass, const world::WorldData &simulatedWorld);
    ...
    std::vector<DefenderBot> decidePositions(const std::vector<DefenderBot> &lockedDefenders, std::vector<int> freeRobots);
    std::tuple<bool, int, std::vector<int>> decideLockedPositions(const std::vector<DefenderBot> &lockedDefenders, std::vector<int> freeRobots);
