...
namespace rtt::ai::world {
    ...
    class WorldData {
        ...
        public:
        WorldData() = default;
        explicit WorldData(const proto::World &copy) : time(copy.time()) {
        ...
        explicit WorldData(const std::vector<RobotPtr> &copyUs, const std::vector<RobotPtr> &copyThem, const BallPtr &copyBall, double time) : time(time) {
        ...
        explicit WorldData(const WorldDataPtr &worldDataPtr) : WorldData(*worldDataPtr) {}
        WorldData(const WorldData &worldData) : WorldData(worldData.us, worldData.them, worldData.ball, worldData.time) {}
    ...
    class WorldBuffer {
        ...
        public:
        explicit WorldBuffer(unsigned int size = 20) : size(size), amountOfWorlds(0) {
            worldBuffer = new WorldData[size];
            lastIndex = 0;
        }

        ~WorldBuffer() { delete[] worldBuffer; }
